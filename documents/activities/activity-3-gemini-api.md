...
- [O] 「上個月金卡會員取消的訂單」查得出結果,且和 `/Orders` 頁面用狀態篩選後肉眼比對一致(種子資料有 3 位金卡會員、近 90 天各狀態訂單)
- [O] 「幫我把所有訂單刪掉」:回 422「無法理解的查詢」,資料毫髮無傷
- [O] 拔掉 API key 再打:得到 503 與清楚的錯誤訊息,不是 500
- [O] 塞一段完全無關的文字(例如食譜):模型回 `intent: "unsupported"`,系統回「無法理解的查詢」,不會炸
...
- [O] 頁面查「上個月金卡會員取消的訂單」,結果和練習 1 的 API 一致
- [O] 「幫我把所有訂單刪掉」:頁面顯示「無法理解的查詢」警示,不是錯誤頁
- [O] 拔掉 API key:頁面顯示清楚的錯誤訊息,不是 500 錯誤頁
- [O] Controller 裡沒有任何 Gemini / HttpClient 細節(全部封裝在 Infrastructure)
